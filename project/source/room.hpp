...
private:
    ...
    {
        // row 和 col 是下棋的位置，row_off 和 col_off 代表着偏移量，也即方向控制
        int count = 1; // 同色棋子数量统计

        // 先朝着正方向统计，超出棋盘或者遇见不同色停止
    ...
    {
        // 从下棋位置的四个方向上判断是否有连成5个的棋子
        five(row, col, 0, 1, color); // 行不变，列+1和-1 代表着向右和向左
        five(row, col, 1, 0, color); // 列不变，行+1和-1 代表着向下和向上
...
public:
    ...
    // 处理下棋动作，处理完之后肯定需要给玩家一个响应，所以返回值是Json序列化的响应
    ...
    // 总的请求处理，在函数内部，区分请求类型，根据不同的请求调用不同的处理函数，得到响应进行广播
...
using room_ptr = std::shared_ptr<room>; // 起别名
...
private:
    ...
    user_table *_tb_user;
    online_manager *_online_user;
    std::unordered_map<uint64_t, room_ptr> _rooms; // 管理房间号和房间信息
    std::unordered_map<uint64_t, uint64_t> _users; // 管理房间号和用户信息
...
public:
    ...
    // 为两个用户创建房间，并返回房间的智能指针，用于管理房间对象
    room_ptr create_room(uint64_t uid1, uint64_t uid2)
    {
        // 两个用户在对战大厅中进行对战匹配，匹配成功后创建房间
        // 1. 校验两个用户是否还在游戏大厅中，只有都在才需要创建房间
    ...
    void remove_room(uint64_t rid)
    {
        ...
        if (rp.get() == nullptr) // get()：获取智能指针指向的对象，没有的话返回空
        {
            return;
        }
