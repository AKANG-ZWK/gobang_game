...
{
private:
    // 需要注意的是： C++中成员变量是按照定义顺序定义初始化的，初始化列表需要和定义顺序保持一致
...
private:
    ...
    {
        // 静态资源请求处理
        // 1. 获取到请求uri，了解客户端请求的页面文件名称
        websocketpp::http::parser::request req = conn->get_request();
        std::string uri = req.get_uri(); // 这个函数会将html页面中所有需要的资源地址都获取到
        ...
        // 5. 设置响应正文
        conn->set_body(body);
        conn->set_status(websocketpp::http::status_code::ok); // 默认状态下响应是500，因此需要手动设置为ok
    }
    ...
    void reg(wsserver_t::connection_ptr &conn)
    {
        ...
        {
            DLOG("反序列化注册信息失败");
            return http_resp(conn, false, websocketpp::http::status_code::bad_request, "请求的正文格式错误"); // 执行后的返回值还是return，因此可以返回，作为函数的终结
        }

        // 3. 进行数据库的用户新增操作
        // if (login_info["username"].isNull() || login_info["password"].isNull())
        if ((login_info["username"].asString() == "") || (login_info["password"].asString() == ""))
        {
            // DLOG("用户名密码不完整");
            DLOG("用户名或密码不能为空");
        ...
        {
            DLOG("向数据库插入数据失败");
            return http_resp(conn, false, websocketpp::http::status_code::bad_request, "用户名已经被占用或不合理!");
        }
        //  如果成功了，则返回200
        DLOG("有新用户注册: username是%s", login_info["username"].asCString());
    ...
    void login(wsserver_t::connection_ptr& conn)
    {
        ...
        // 2. 校验正文完整性，进行数据库的用户信息验证 (如果验证失败，则返回400)
        // if (login_info["username"].isNull() || login_info["password"].isNull())
        if ((login_info["username"].asString() == "") || (login_info["password"].asString() == ""))
        {
            // DLOG("用户名密码不完整");
    ...
    void info(wsserver_t::connection_ptr& conn)
    {
        ...
        if (ssp.get() == nullptr)
        {
            ...
            return http_resp(conn, true, websocketpp::http::status_code::bad_request, "找不到session信息，请重新登录");
        }
    ...
    void http_callback(websocketpp::connection_hdl hdl) // 负责处理http请求的处理
    {
    ...
    void wsopen_game_room(wsserver_t::connection_ptr conn)
    {
        ...
        // 2. 当前用户是否已经在游戏房间或者游戏大厅中 -- 在线用户管理
        // 进入游戏房间的时候，用户就从hall中离开了，所以调用了wsclose，将用户从hall表中移除了已经
        if (_om.is_in_game_hall(ssp->get_user()) || _om.is_in_game_room(ssp->get_user()))
        {
    ...
    void wsopen_callback(websocketpp::connection_hdl hdl)
    {
        ...
        wsserver_t::connection_ptr conn = _wssrv.get_con_from_hdl(hdl);
        websocketpp::http::parser::request req = conn->get_request(); // 获取websocket请求
    ...
    void wsclose_game_room(wsserver_t::connection_ptr conn)
    {
        ...
        // 2. 将session恢复生命周期的管理，设置定时销户
        _sm.set_session_expire_time(ssp->ssid(), SESSION_TIMEOUT);
        // 3. 将玩家从游戏房间中移除，房间中所有用户推出了就销毁房间
    ...
    void wsmsg_game_hall(wsserver_t::connection_ptr conn, wsserver_t::message_ptr msg)
    {
        ...
        // 2. 获取请求信息
        std::string req_body = msg->get_payload(); // get_payload() ：获取websocket数据
        /*
            get_payload()函数是一个用于获取WebSocket消息有效负载的方法。该函数通常在处理接收到的WebSocket消息时使用。
            get_payload()函数通常返回一个string类型的值，其中包含了WebSocket消息的有效负载内容。有效负载是指WebSocket消息
            除去帧头和帧尾的主体部分，其中包含了实际传输的数据。
        */
        Json::Value req_json;
...
public:
        ...
        _rm(&_ut, &_om), _sm(&_wssrv), _mm(&_rm, &_ut, &_om)
    {
        ...
        _wssrv.set_reuse_addr(true); // 设置端口号可重新绑定
